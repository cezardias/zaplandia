...
let UsageService = UsageService_1 = class UsageService {
    ...
    async checkAndReserve(tenantId, instanceName, feature, amount, limitOverride) {
    ...
    async getRemainingQuota(tenantId, instanceName, feature, limitOverride) {
        const today = this.getTodayString();
        const limit = limitOverride !== undefined ? limitOverride : (this.LIMITS[feature] || 999999);
    ...
    async resetUsage(tenantId, instanceName, feature) {
        const today = this.getTodayString();
