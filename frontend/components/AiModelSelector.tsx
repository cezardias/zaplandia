...
export default function AiModelSelector({ value, onChange, token, className = "" }: AiModelSelectorProps) {
    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            ...
            {isOpen && (
                <div className={`absolute z-[100] w-full bg-[#1e293b] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 ${direction === 'up' ? 'bottom-[calc(100%+8px)]' : 'mt-2'
                    }`}>
